...
final caseDetailFromRepoProvider = FutureProvider\.autoDispose.family<Case?, int>((ref, caseId) {
  return ref.watch(caseRepositoryProvider).getCaseById(caseId);
});

/// أطراف الدعوى من المستودع
final casePartiesProvider = StreamProvider\.autoDispose.family<List<CaseParty>, int>((ref, caseId) {
  return ref.watch(caseRepositoryProvider).watchCaseParties(caseId);
});

/// جلسات الدعوى من المستودع
final caseSessionsProvider = StreamProvider\.autoDispose.family<List<CaseSession>, int>((ref, caseId) {
  return ref.watch(caseRepositoryProvider).watchCaseSessions(caseId);
});

/// مراحل الدعوى من المستودع
final casePhasesProvider = StreamProvider\.autoDispose.family<List<CasePhase>, int>((ref, caseId) {
  return ref.watch(caseRepositoryProvider).watchCasePhases(caseId);
});

/// النواقص المفتوحة لدعوى معينة
final caseOpenDeficienciesProvider = StreamProvider\.autoDispose.family<List<Deficiency>, int>((ref, caseId) {
...
final allPersonsProvider = StreamProvider\.autoDispose.family<List<PersonEntity>, PersonType?>((ref, type) {
...
final tasksByDateProvider = StreamProvider\.autoDispose.family<List<DailyTask>, DateTime?>((ref, date) {
  final target = date ?? DateTime.now();
  return ref.watch(taskRepositoryProvider).watchTasksByDate(target);
});

/// قائمة النواقص المفتوحة في المكتب
final openDeficienciesProvider = StreamProvider\.autoDispose.family<List<Deficiency>, ({EntityType? type, int? id})?>((ref, filter) {
  return ref.watch(taskRepositoryProvider).watchOpenDeficiencies(
...
final activeCourtsProvider = StreamProvider\.autoDispose.family<List<Court>, String?>((ref, type) {
